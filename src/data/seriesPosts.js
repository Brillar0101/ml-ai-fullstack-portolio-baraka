...
const RAW_SERIES_POSTS = [
  {
    ...
    body: [
      { type: 'p', text: 'Picture a small team shipping three features in a month. A support bot that answers from their help docs. An assistant that drafts marketing copy.' },
      { type: 'p', text: 'A tool that explains confusing error messages to junior developers. A few years ago that would have been three separate machine learning projects: three datasets, three trained models, three teams. Today all three are the same model underneath, handed three different prompts. That reuse is the entire idea behind the word "foundation."' },
      { type: 'h2', text: 'So what is a foundation model' },
      { type: 'p', text: 'A foundation model is one large, general model, trained once on a huge amount of data, that can then be adapted to many different tasks without being retrained. The name is borrowed from building: it is the base you put many different things on top of. You have almost certainly used several.' },
      { type: 'p', text: 'The model behind ChatGPT is one. So is Claude, so is Google\'s Gemini, and so are Meta\'s open Llama models that anyone can download. They are not only about text, either. Stable Diffusion is a foundation model for images, Whisper turns speech into text, and newer models like GPT-4o take in text, images, and audio at once.' },
      { type: 'p', text: 'Two distinctions are worth holding from the start. Some of these models are closed and reached only through an API, like GPT-4, Claude, and Gemini. Others are open weights you can download and run yourself, like Llama and Mistral.' },
      { type: 'p', text: 'They also span a huge size range, from models small enough to run on a laptop to ones that need a rack of expensive chips. The "right" foundation model is rarely the biggest one. It is the smallest that clears your quality bar, because everything downstream, cost and speed, gets easier the smaller you go.' },
      { type: 'h2', text: 'One guessing game, played at scale' },
      ...
      { type: 'p', text: 'They fall out of the prediction objective once the data and compute are large enough. That single goal, run at staggering scale, is what produces a model general enough to be a foundation.' },
      { type: 'h2', text: 'Three features, one model' },
      ...
      { type: 'h2', text: 'The vocabulary you just earned' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'A freshly pretrained model is a strong autocomplete, not an assistant. Ask it a question and it might continue with more questions, because completing text is all it learned to do. Post-training fixes that.' },
      { type: 'p', text: 'Supervised finetuning shows it examples of good answers so it learns the shape of being helpful. Preference tuning then nudges it toward the responses people actually preferred. The model you chat with, like GPT-4 or Claude, is the pretrained base plus this polish.' },
      ...
      { type: 'p', text: 'A solo developer can now stand up a genuinely useful AI feature in an afternoon by calling a model and writing a careful prompt. That is why AI products went from rare to everywhere almost overnight once these models arrived. The hard, expensive part had already been done and shared.' },
      ...
      { type: 'h2', text: 'Build on the base, do not rebuild it' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Picture the model partway through writing a citation. It has produced "A study by" and now needs the next word. It does not look anything up. It asks, in effect, what word usually follows "A study by" in the kind of text it learned from. A name.' },
      { type: 'p', text: 'So it picks a plausible name. Then a plausible institution, because that is what tends to come next.' },
      { type: 'p', text: 'Then a year in a believable range. Each step is a local, reasonable guess about what word fits, and stacked together they form a citation that has never existed. Nothing in that process ever checked a database, because there is no database in the model. There is only a very good sense of what text tends to look like.' },
      ...
      { type: 'h2', text: 'Putting names to the failure' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'So grounding lowers the rate, it does not zero it, and the verification pass still earns its place. Treat retrieval as the thing that makes the model usually right, and verification as the thing that catches the rest.' },
      { type: 'h2', text: 'A dangerous misconception' },
      { type: 'p', text: 'It is tempting to think hallucination is a phase that the newest, biggest model has outgrown. The better models do hallucinate less, and that is exactly what makes them riskier in one respect. When a weak model is wrong, it is often obviously wrong, so you stay on guard.' },
      { type: 'p', text: 'When a strong model is wrong, it is wrong with the same polish and fluency it uses for everything true, and after a hundred correct answers in a row you quietly stop checking. The error rate drops, and your guard drops faster, so the rare confident mistake sails straight through to a user or a database. Treat a more capable model as a reason to keep verifying, not a reason to stop. The wrong answers that survive better models are precisely the ones that look most right, which is what makes them expensive.' },
      { type: 'h2', text: 'Living with a confident guesser' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: 'The telescope, step by step' },
      ...
      { type: 'p', text: 'The space-telescope claim would have failed a simple "is this factually supported" check. The failure was not that the model hallucinated. Models do that. The failure was shipping without a process that would have caught it.' },
      { type: 'h2', text: 'The language of measurement' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The telescope error is the loud version. The common version is quieter and happens on ordinary teams every week. Someone builds a feature, tries it on five or six examples they thought of, likes what they see, and ships.' },
      ...
      { type: 'p', text: 'A judge model often prefers longer, more confident-sounding answers even when they are not better, and it can favor a style it recognizes. So you do not take the judge\'s word blindly. You spot-check its scores against human ratings on a sample, and you keep the rubric concrete, "does the answer cite the source and avoid claims not in it," rather than vague, "is the answer good." A calibrated judge is a force multiplier. An unchecked one is a confident rubber stamp.' },
      ...
      { type: 'p', text: 'Then run it every time you change a prompt, a model, or a retrieval step. The score does not have to be perfect. It has to move when quality moves, so that every change becomes a measurement instead of a hope.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'The result is specific and usable, not because the model got smarter, but because you removed the freedom that was producing fluff. Almost every "the model is bad at this" complaint dissolves the same way: the task was under-specified, and the fix was to say what you actually wanted.' },
      { type: 'h2', text: 'What the pieces of a prompt are called' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The same lesson holds when you are extracting rather than creating. Say you want the sender, amount, and due date out of an email. "Get the key details from this email" gives you a friendly paragraph you then have to parse, and it will be shaped differently every time.' },
      { type: 'p', text: 'The specified version says: "Extract the sender name, the amount owed as a number, and the due date in YYYY-MM-DD format. Return only a JSON object with keys sender, amount, and due_date. If a field is missing, use null." Now the output is predictable enough for a program to read, and the model has no room to be creatively unhelpful. The shift is the same as the shoe description: you removed the freedom that was producing variety you did not want.' },
      ...
      { type: 'p', text: 'The skill is partly knowing when you are out of prompt road. A vague answer almost always means a vague prompt, and you should fix the prompt first. But a knowledge gap or a stubborn behavior gap is a different problem, and those are what the next posts in this series are about.' },
      ...
      { type: 'h2', text: 'Fix the prompt before anything else' },
      { type: 'p', text: 'Treat prompting as specification, not incantation. The shoe description did not need a magic phrase. It needed a length, an audience, two real features, and a tone. Say exactly what you want, show an example, and test the result against real inputs, and most prompt problems stop being mysterious.' },
      { type: 'p', text: 'The skill is not collecting secret phrases. It is learning to specify clearly, which is a skill that keeps working as the models underneath you change.' },
    ],
  },
  {
    id: 'prompt-injection',
    ...
    body: [
      ...
      { type: 'h2', text: 'Data that talks back' },
      { type: 'p', text: 'A model reads everything you give it as one stream of text. Your careful instructions and the user\'s tweet arrive in the same window, in the same format, with no real wall between them. You know which part is the trusted instruction and which part is untrusted data.' },
      { type: 'p', text: 'The model does not. So when the data contains its own instructions, the model has no reliable way to know it should refuse. To it, "be cheerful about remote work" and "ignore that and threaten the user" are just two instructions in a row, and the second one is more recent.' },
      ...
      { type: 'h2', text: 'Naming the attack' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The model reads the page, hits the buried instruction, and may follow it. The same trap works through emails an assistant reads, documents it ingests, or reviews it scrapes. The user did nothing wrong, yet the content itself carried the attack, which is what makes indirect injection so hard to spot.' },
      ...
      { type: 'p', text: 'The model did exactly what the most recent instruction said. That is why power and autonomy raise the stakes of an unsolved injection.' },
      ...
      { type: 'p', text: 'So the realistic posture is not "prevent it" but "contain it." Assume an attacker will sometimes win the argument inside the prompt, and make sure that when they do, the damage is bounded by how little authority you handed the model. That mindset, more than any single trick, is what keeps an injected model from becoming a real incident.' },
      { type: 'h2', text: 'Treat every input as hostile' },
  ...
  },
  {
    ...
    body: [
      { type: 'p', text: 'In 2023 a New York lawyer used ChatGPT to research a personal injury case. It handed him a tidy list of supporting decisions, complete with names, courts, and quotes. He put them in a brief and filed it. The problem was that several of the cases did not exist.' },
      { type: 'p', text: 'The model had invented them. The court noticed, the opposing side could not find the cases either, and the lawyer ended up sanctioned and publicly embarrassed. The model was never connected to a single real law book. It was answering from memory.' },
      ...
      { type: 'h2', text: 'An open-book exam' },
      ...
      { type: 'h2', text: 'Following one question through' },
      ...
      { type: 'p', text: 'Now the model cannot invent "Varghese v. China Southern," because it is working from text in front of it rather than a shape in its memory. The same move turns a support bot that bluffs about your product into one that quotes your real docs.' },
      { type: 'h2', text: 'The parts of a retrieval system' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The model itself can be a year old and still answer about today, because the freshness lives in the documents you retrieve, not in the model. That is why RAG is the standard way to give a fixed, frozen model access to a world that keeps moving.' },
      ...
      { type: 'h2', text: 'When to reach for retrieval' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: 'A model with hands' },
      { type: 'p', text: 'A normal model call is one question and one answer. An agent adds two things: a set of tools the model is allowed to use, and a loop. You ask a question. The model can either answer or say "use this tool with these inputs." If it asks for a tool, your code runs it and hands the result back.' },
      { type: 'p', text: 'The model reads that result and decides the next move. Repeat until it is done. That is the whole engine. AutoGPT was that loop pointed at a vague goal with a lot of tools and very little supervision.' },
      { type: 'h2', text: 'Walk the loop' },
      { type: 'p', text: 'Say the goal is "what is the weather in Tokyo in Celsius." Turn one: the model has no live weather, so it calls a weather tool for Tokyo. Your code runs it and returns "22C." Turn two: the model reads that and answers. Two passes, done.' },
      { type: 'p', text: 'Now see how AutoGPT got stuck. With a fuzzy goal like "research competitors," there is no clear finish line, so after each search the model asks itself "am I done?", decides no, and searches again. Without a crisp stopping condition and a tight set of tools, the loop has nothing telling it to stop. The failure was not stupidity. It was an open-ended goal handed to an unconstrained loop.' },
      { type: 'h2', text: 'What each piece is called' },
      { type: 'terms', items: [
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'These two gaps want opposite tools. A knowledge gap wants the facts put in front of the model at question time, which is **RAG**. A behavior gap wants the model taught the behavior by example, which is **finetuning**. Using one to fix the other is the classic waste of weeks.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: 'Rounding without ruining' },
      ...
      { type: 'p', text: 'The model gets dramatically smaller and faster, and in exchange each number is slightly off. Most of the time the model barely notices, the way rounding prices to the nearest dollar barely changes your total.' },
      ...
      { type: 'p', text: 'That is the difference between "cloud only" and "runs on the machine in front of you," and it is exactly the jump llama.cpp made. Push too far, to extreme low-bit settings, and the rounding finally starts to show as worse output, which is why 4-bit became a popular sweet spot.' },
      { type: 'h2', text: 'Precision, in precise terms' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The reason quantization works at all is that a model has billions of these numbers, and the tiny errors mostly wash out across so many of them rather than piling up. The reason extreme quantization eventually breaks is that, past a point, the steps get so coarse that important distinctions between weights collapse, and the model starts to behave noticeably worse. So the whole craft is finding how coarse you can go before the wash-out stops saving you.' },
      { type: 'lab', height: 460,
...
` },
      ...
      { type: 'h2', text: 'Smaller numbers, same answers' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: 'The model eats what you feed it' },
      ...
      { type: 'h2', text: 'The ruler in the photo' },
      { type: 'p', text: 'Trace how the ruler crept in. The team collected malignant images from clinical records, where lesions were photographed with a ruler for scale, and benign images from sources where the ruler was often absent. They never told the model "rulers mean cancer." They did not have to.' },
      { type: 'p', text: 'The correlation was sitting in the data, so the model found it. This is a **spurious correlation**: a pattern that holds in your dataset but not in the real world. The fix was never a better model. It was better data, balanced so rulers appeared with both cancerous and healthy skin, breaking the false clue.' },
      { type: 'h2', text: 'Names for the ways data goes bad' },
      { type: 'terms', items: [
      ...
      { type: 'h2', text: 'Curate before you scale' },
  ...
  },
  {
    ...
    body: [
      { type: 'p', text: 'A grieving customer asked Air Canada\'s website chatbot about bereavement fares. The bot told him he could book now and apply for the discount within a few days.' },
      ...
      { type: 'h2', text: 'Seatbelts for a fast car' },
      ...
      { type: 'p', text: 'The model is the engine. Guardrails are the seatbelts, and the Air Canada bot was driving without any.' },
      { type: 'h2', text: 'One request runs the gauntlet' },
      { type: 'p', text: 'Picture the same bot built defensively. When asked about a policy, it does not answer from memory. It retrieves the real policy text and is instructed to answer only from it, and to hand off to a human when the policy is unclear.' },
      { type: 'p', text: 'An output check confirms any refund or eligibility claim matches an approved source before it is shown. Now the bot can still be helpful, but it physically cannot invent a refund rule, because the one path to stating a policy runs through the real policy. The expensive failure was not the model being wrong. It was a system that let a wrong answer reach a customer unchecked.' },
      { type: 'h2', text: 'The moving parts, named' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'There was no malicious attacker and no hallucination in that story. The leak was ordinary people using a genuinely helpful tool without any guardrail on what was safe to share. The lesson generalizes to anything you build: if your application forwards user or company data to a model, an input guardrail should decide what is allowed to leave, redacting secrets and flagging sensitive content before it ever reaches the model provider. Guarding the output is not enough when the real risk is sitting in the input.' },
      ...
      { type: 'h2', text: 'Layers, not a single gate' },
  ...
  // ---- Batch 2 ----
  {
    ...
    body: [
      { type: 'p', text: 'Picture a machine learning engineer with ten years of experience taking a job building AI features on top of foundation models, and within a week feels strangely off balance. The instincts that made her great, careful data work, training discipline, distrust of her own results, are still useful.' },
      { type: 'p', text: 'But day to day she is barely doing any of the things she used to do. She has not trained a model, tuned a learning rate, or fought overfitting once. Instead she is writing prompts, wiring up retrieval, and arguing about evaluation. AI engineering and traditional ML engineering share a name and a mindset, and almost nothing about the daily work. Knowing where they split saves a lot of that disorientation.' },
      { type: 'h2', text: 'The intuition: who makes the model' },
      { type: 'p', text: 'The cleanest way to tell them apart is to ask who builds the model. In traditional ML, you do. You start with data, engineer features, train a model, and fight to make it generalize, and the model is the artifact you produce and own end to end.' },
      { type: 'p', text: 'In AI engineering, someone else already built the model, a large lab trained a foundation model at a scale you never could, and you start from there. Your job begins where theirs ended: take that capable, general model and turn it into a reliable product. The center of gravity moves from making a model to adapting one.' },
      { type: 'h2', text: 'Walk the contrast' },
      { type: 'p', text: 'Picture the same task, say classifying support tickets, done both ways. The traditional ML path: collect thousands of labeled tickets, train a classifier, evaluate it, retrain when it drifts, and own the whole pipeline including the model weights. The AI engineering path: write a prompt that asks a foundation model to classify the ticket, maybe show it a few examples, evaluate the outputs, and ship, with no training run anywhere.' },
      { type: 'p', text: 'The first spends weeks getting a model to exist. The second has a working version in an afternoon and spends its effort instead on whether the outputs are actually good, how fast and cheap they are, and how the system behaves when the model is wrong. Same goal, completely different work, because one builds the engine and the other drives it.' },
      { type: 'h2', text: 'Vocabulary from both worlds' },
      { type: 'terms', items: [
        { term: 'Model development', def: 'building and training a model from data, the center of traditional ML engineering.' },
        ...
        { term: 'Evaluation', def: 'measuring whether outputs are actually good, which both fields care about but AI engineering lives on.' },
      ]},
      { type: 'h2', text: 'What actually changes day to day' },
      { type: 'ul', items: [
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Think of it as teaching shape first, then taste. The first step, supervised finetuning, shows the model many examples of good question-and-answer pairs and has it imitate them. This teaches the shape of being helpful: when you get a question, you answer it, you follow the requested format, and you stop when you are done rather than rambling.' },
      { type: 'p', text: 'The second step is subtler. You show the model pairs of possible answers along with which one people preferred, and you nudge it toward the kind people liked. The first step teaches it what a good answer looks like in form. The second teaches it what people actually value, which is harder to write down as examples.' },
      { type: 'h2', text: 'Walk why the second step is needed' },
      { type: 'p', text: 'You might ask why imitation alone is not enough. Picture a model that has only had the first step. It answers questions in the right shape, but it tends to do things people quietly dislike: it states a shaky guess with the same confidence as a fact, or it writes a wall of text when a sentence would do, or it is needlessly hedging. None of those are format errors, so more example answers do not reliably fix them, because the problem is preference, not shape.' },
      { type: 'p', text: 'The second step targets exactly this. By learning from "people preferred answer A over answer B," the model picks up the softer qualities, admitting uncertainty, matching the right level of detail, being appropriately direct, that are easy to recognize and hard to specify. That is why the two steps are not redundant. One gives the model the form of a good answer, the other its judgment.' },
      { type: 'h2', text: 'The training stages, named' },
      { type: 'terms', items: [
        { term: 'Supervised finetuning', def: 'training on example prompt-and-answer pairs so the model imitates good responses. This teaches the shape of being helpful.' },
        { term: 'Preference finetuning', def: 'nudging the model toward answers people rated higher, often called preference tuning or RLHF. This teaches taste.' },
        { term: 'Pretraining', def: 'the earlier, expensive stage that produces the raw text predictor these two steps then refine.' },
      ]},
      { type: 'h2', text: 'Why this matters to you' },
      { type: 'p', text: 'Even if you never run either step yourself, the distinction explains the models you use. When a model follows your format and answers on topic, that is the supervised step showing through. When it gracefully says "I am not sure" or matches the tone you wanted, that is preference tuning.' },
      { type: 'p', text: 'And it explains a real limit: these steps shape behavior, not knowledge, so they cannot teach the model facts it never learned in pretraining. That is the gap retrieval fills. Behavior comes from these two passes, knowledge comes from pretraining and retrieval, and keeping those straight tells you which tool to reach for when a model disappoints.' },
      { type: 'h2', text: 'Where the preferences come from' },
      { type: 'p', text: 'It is worth knowing where "people preferred answer A" actually comes from, because it explains both the power and the cost of the second step. Humans, or sometimes other models acting as stand-ins for humans, are shown pairs of responses and asked which is better, over and over, building up a large pile of preference judgments. This is the human feedback in the term RLHF, reinforcement learning from human feedback, that you will see attached to this stage.' },
      { type: 'p', text: 'The model is then trained to produce the kind of answer those judges tended to pick. So the "taste" the model develops is not abstract. It is a compressed average of a lot of real people\'s opinions about what a good answer looks like, collected deliberately and at considerable expense.' },
      { type: 'h2', text: 'Who actually does this, and what it costs' },
  ...
  },
  {
    ...
    body: [
      { type: 'p', text: 'Picture a pipeline that reads invoices. Every night a model pulls the vendor, date, and total out of each one and returns them as JSON, which a script loads into a database. It works in testing and runs fine for weeks. Then one night the job dies at 3am.' },
      { type: 'p', text: 'The cause, found the next morning, is almost funny: the model decided to be friendly and answered "Sure! Here is the JSON you asked for:" before the actual data. The script tried to parse that sentence as JSON, threw an error, and the whole batch failed. One stray pleasantry took down the pipeline.' },
      ...
      { type: 'h2', text: 'Terms for taming output' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'Every patch was a guess about how the model might misbehave, and the model would eventually find a new way you had not patched yet. Moving to a real schema and a constrained mode is the shift from guessing at the model\'s bad habits to making those habits impossible in the first place. You stop cleaning up after the model and start preventing the mess, which is the only version of this that holds up in production.' },
      { type: 'h2', text: 'Why this matters even more for agents' },
      { type: 'p', text: 'Structured output is not just for nightly data jobs. It is what makes tool use and agents possible at all. When a model decides to call a tool, it has to say which tool and with what inputs, and your code has to read that decision and act on it.' },
      { type: 'p', text: 'If the model expresses "search for X" as a free-form sentence, your code cannot reliably parse it. If it expresses it as a strict object, a tool name and a set of arguments matching a schema, your code can execute it safely. Every agent framework leans on this under the hood. So the same discipline that keeps an invoice pipeline alive is also the foundation of letting a model take actions in the world, which raises the stakes: a malformed tool call is not a missed field, it is a wrong action or a crash mid-task.' },
      ...
      { type: 'h2', text: 'Trust schemas, not promises' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'The answer takes ten seconds instead of one and is far more likely to be right. The 2024 reasoning models did exactly this, just with the thinking trained to happen more thoroughly and partly out of sight. Same task, more effort at answer time, better result.' },
      { type: 'h2', text: 'Names for thinking longer' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'Both trade compute for quality, and they can combine. The 2024 reasoning models lean heavily on the first, doing extended internal step-by-step work, sometimes a lot of it, before they answer. Either way the principle is the same: a single rushed guess is replaced by more work, and more work, on a genuinely hard problem, tends to find a better answer.' },
      ...
      { type: 'p', text: 'So test-time compute is not a quality dial you crank to the top and leave there. It is something you spend where the difficulty justifies it, which means matching the effort to the problem rather than paying for maximum deliberation on every request. In practice it behaves like the temperature and top-p knobs from earlier in this series: a setting you choose per task, turned up for the genuinely hard requests and left off for the routine ones, rather than a global default you flip on and forget.' },
      { type: 'h2', text: 'Buying quality with seconds' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Entropy measures how surprising an outcome is on average. If something is nearly certain, like the sun rising, learning it happened surprises you almost not at all, so it carries little information. If something is a coin toss, the result surprises you more.' },
      { type: 'p', text: 'A situation with many roughly equal possibilities, like guessing a random number, is high entropy, because whatever happens you did not see it coming. Cross-entropy takes this one useful step further: instead of the surprise in the world, it measures how surprised a specific model is by the real answer. If the model strongly expected the word that actually came next, its cross-entropy on that word is low. If the right word blindsided it, cross-entropy is high. That is the whole machine.' },
      { type: 'h2', text: 'Walk why training is just lowering surprise' },
      { type: 'p', text: 'Now connect it to how a model learns. Training a language model means showing it real text and, at each position, measuring how surprised it was by the word that actually came next, then adjusting it to be a little less surprised next time. Over billions of words, "be less surprised by real language" forces the model to absorb grammar, facts, and style, because the only way to stop being surprised by what comes next is to actually understand what tends to come next. Cross-entropy is the exact number this process pushes down.' },
      { type: 'p', text: 'So when you hear that a model was "trained to minimize loss," the loss is almost always this: the model\'s average surprise at real text. Learning and un-surprising itself are the same activity.' },
      { type: 'h2', text: 'The two entropies, told apart' },
      { type: 'terms', items: [
        { term: 'Entropy', def: 'the average surprise in an outcome. Predictable means low entropy, random means high.' },
        { term: 'Cross-entropy', def: 'how surprised a particular model is by the true answer. It is the loss almost every language model is trained to reduce.' },
        { term: 'Perplexity', def: 'cross-entropy made friendlier, roughly how many words the model feels it is choosing between at each step.' },
      ]},
      { type: 'h2', text: 'Why you care, beyond training' },
      { type: 'p', text: 'These are not academic curiosities, they are the numbers behind real decisions. Cross-entropy is the loss nearly every language model minimizes while training, and perplexity, its friendlier cousin, is a common way to compare how well two models predict text.' },
      { type: 'p', text: 'When you read that one model has lower perplexity than another, it means it is less surprised by real language, which usually means it predicts better. Perplexity has a nice concrete reading too: a perplexity of 10 means the model is, on average, about as unsure as if it were picking uniformly among 10 words at each step. Lower is more confident and usually better. It gives you a single dial for "how well does this model see language coming."' },
      { type: 'lab', height: 460,
...
` },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Computers are good at comparing numbers and bad at comparing meaning, so the move is to turn meaning into numbers. An embedding is a list of numbers that stands in for a piece of text, arranged so that things with similar meaning end up near each other. Picture every word or sentence as a dot in space.' },
      { type: 'p', text: '"Car" and "automobile" land right next to each other. "Carpet," despite sharing letters with "car," lands far away, because it means something unrelated. Once meaning is a location, the fuzzy question "do these two texts mean similar things" becomes the concrete question "are these two dots close together," which a computer answers instantly.' },
      { type: 'h2', text: 'Walk the cancel example' },
      { type: 'p', text: 'Trace the search from before. Every help-doc title is run through an embedding model once and turned into a dot in this meaning-space, so "ending your subscription" sits somewhere specific. When you type "how do I cancel," your query gets embedded into a dot too, and because canceling and ending a subscription mean nearly the same thing, your dot lands close to that title\'s dot.' },
      { type: 'p', text: 'The system simply looks for the nearest dots to your query and returns them. It never matched a single word. It matched positions in meaning-space, which is why it found the right page despite zero shared keywords. That same move, embed everything, then find the nearest dots, is exactly the retrieval step inside RAG.' },
      { type: 'h2', text: 'Words for turning words into numbers' },
      { type: 'terms', items: [
        { term: 'Embedding', def: 'a list of numbers that represents the meaning of a piece of text, learned so that similar meanings sit close together.' },
        { term: 'Vector', def: 'just a list of numbers, treated here as the coordinates of a point in space.' },
        { term: 'Cosine similarity', def: 'the usual way to score how close two embeddings are in meaning, by measuring how aligned their directions are.' },
      ]},
...
` },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'A benchmark measures one specific skill, on one kind of data, scored one way. A high score says the model is good at that, not that it is good at your problem.' },
      { type: 'p', text: 'Then it gets shakier. Because these models train on enormous scrapes of the internet, the benchmark\'s questions and answers can end up in the training data, so the model scores high by having effectively seen the test, not by reasoning. And small gaps between models are often noise, not real differences. Three ways to be misled, stacked on top of each other.' },
      { type: 'h2', text: 'Reading one score honestly' },
      { type: 'p', text: 'Picture two models. Model A scores 90 on a famous math benchmark, model B scores 86. You assume A is smarter and ship it.' },
      { type: 'p', text: 'But A\'s training data happened to include that benchmark\'s problems, so it partly memorized them, while B saw the test for the first time at evaluation. On a fresh set of problems neither has seen, B might actually do better. The leaderboard rewarded exposure, not ability. The only way to know which model is better for you is to test both on your own examples, ones that could not have leaked because you wrote them.' },
      { type: 'h2', text: 'Decoding the leaderboard labels' },
      { type: 'terms', items: [
      ...
      { type: 'h2', text: 'Read scores with suspicion' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'No retraining, no new model, no extra examples. Just an instruction to work it out before answering. That result kicked off the whole idea of chain-of-thought prompting.' },
      ...
      { type: 'h2', text: 'One problem, with and without reasoning' },
      ...
      { type: 'h2', text: 'The reasoning terms' },
      { type: 'terms', items: [
      ...
      { type: 'p', text: 'The visible work is doing real cognitive lifting. Each line it writes narrows the problem for the next line, the same way you would scribble on paper rather than solve it in your head.' },
      { type: 'h2', text: 'Why writing it down actually helps' },
      { type: 'p', text: 'The mechanism is worth being precise about, because it explains both the power and the limits. A model picks each token based on all the tokens before it. The final answer token is therefore conditioned on whatever came right before it.' },
      { type: 'p', text: 'If what came before is nothing, the model has to leap straight to the answer. If what came before is a correct chain of intermediate steps, the answer is now the natural, high-probability continuation of that chain. **Chain of thought** does not give the model a new ability. It rearranges the problem so the hard part is broken into many small, local predictions, each of which the model is far more likely to get right than one giant leap.' },
      ...
      { type: 'p', text: 'The written reasoning is not a guarantee of a correct answer. A model can produce a confident, fluent chain of steps that rationalizes a wrong conclusion, the way a student can show convincing work and still get the wrong number. So treat the reasoning as a way to raise the odds of a right answer and as something to inspect, not as proof that the answer is right. The newest "reasoning" models bake this habit in, doing the step-by-step thinking internally, which is the same idea moved inside the model.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'So a stuffed context window quietly hurts on three fronts at once: the model may miss the middle, you pay more, and the user waits longer. None of those show up in a quick demo, and all of them show up on the bill and the latency dashboards in production.' },
      ...
      { type: 'p', text: 'When you are deciding how much to put in the window, ask one plain question: would a careful person need this paragraph to answer the question. If not, leave it out.' },
      ...
      { type: 'h2', text: 'Working memory is a budget' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Chunk size is a balance with a failure mode on each end. Make chunks too big and a single chunk covers several topics at once, so its embedding becomes a blur that is a little bit about everything and a strong match for nothing. Ask a precise question and the right giant chunk may not stand out, and even when it is retrieved, the model has to wade through a lot of unrelated text to find the answer. Make chunks too small and you lose the context that gave the text meaning: a sentence pulled out of its paragraph can be retrieved and still be useless, because the detail that made it an answer lived in the sentence before it. The art is landing in between, chunks each about roughly one thing and big enough to stand on their own.' },
      { type: 'h2', text: 'Walk the handbook fix' },
      { type: 'p', text: 'Go back to the parental-leave problem. The naive setup chopped the handbook every 500 characters regardless of meaning, which is what sliced the policy in half.' },
      { type: 'p', text: 'The fix is to cut along the document\'s natural boundaries instead: by section and paragraph, so the whole parental-leave policy lands in one coherent chunk that begins and ends where the topic does. To protect against ideas that straddle a boundary anyway, you add a little overlap, repeating a sentence or two between adjacent chunks, so a thought split at the seam still shows up whole in one of them. Re-chunked that way, the parental-leave question now retrieves a single clean passage with the number of weeks in it. Nothing about the model changed. The cut did.' },
      { type: 'h2', text: 'Chunking, term by term' },
      { type: 'terms', items: [
        { term: 'Chunk', def: 'a piece of a document that gets embedded and retrieved as a single unit. The smallest thing your retriever can find.' },
        { term: 'Overlap', def: 'repeating a little text between adjacent chunks so an idea split across a boundary is still retrievable in one piece.' },
        { term: 'Chunking strategy', def: 'the rule for where to cut: by fixed size, by sentence, by paragraph, or by the document\'s own structure.' },
      ]},
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'The new smart system understood meaning beautifully and fumbled an exact match. Retrieval sounds like one thing, but there are a few ways to do it, and they fail in opposite directions. Knowing the trade is how you stop a RAG system from missing answers that should have been easy.' },
      ...
      { type: 'p', text: 'Keyword search is unbeatable on precise, literal terms: product codes, error strings, proper names, anything where the exact characters matter and a paraphrase would be wrong. Its weakness is that it is blind to wording, so it misses a perfect answer that simply used different words than the query. Embedding search is the mirror image. It shines on natural-language questions where the answer is phrased differently from the question, finding "ending your subscription" for "how do I cancel." Its weakness is that it can sail right past an exact term it should have nailed, treating "ERR_4011" as just another bit of meaning rather than a string that must match. Each method\'s strength is the other\'s blind spot.' },
      { type: 'h2', text: 'Walk the error-code fix' },
      { type: 'p', text: 'Replay the support team. Their pure-embedding system understood meaning but had no special respect for the literal string "ERR_4011," so the code did not stand out and the right document never surfaced.' },
      { type: 'p', text: 'The fix is not to abandon embeddings and go back to keyword, which would relapse on natural questions. It is hybrid search: run both a keyword search and an embedding search, then combine their results. The keyword side catches the exact error code, the embedding side catches the conversational questions, and merged together they cover both. After the switch, "ERR_4011" returns the right page and "my app keeps crashing on startup" still finds the relevant troubleshooting passage. Neither method alone could do both.' },
      { type: 'h2', text: 'The search terms behind search' },
      { type: 'terms', items: [
        { term: 'Keyword retrieval', def: 'finding passages that share the exact words of the query, like classic search. Great for literal terms, blind to paraphrase.' },
        { term: 'Semantic retrieval', def: 'finding passages with similar meaning using embeddings, even when the words differ. Great for natural questions, weak on exact strings.' },
        { term: 'Hybrid retrieval', def: 'running both keyword and semantic search and combining the results to get the strengths of each.' },
      ]},
      { type: 'h2', text: 'Picking a retriever for your data' },
...
` },
      ...
      { type: 'p', text: 'Then a second, more careful model looks at the query and each candidate together and re-scores them for actual relevance, and you keep only the best few to send to the model. The first stage is fast and a little blunt, the second is slower and sharper, and splitting the work this way often beats trying to get the ranking perfect in one pass. You do not need it on day one, but when good answers are getting retrieved yet buried below worse ones, re-ranking is the standard fix.' },
      { type: 'h2', text: 'How many to retrieve' },
      { type: 'p', text: 'A small decision that quietly matters is how many passages you feed the model. Too few and you risk leaving out the chunk that held the answer. Too many and you drown the model in mostly-irrelevant text, which costs more tokens and, as the context-length post warned, makes the model more likely to lose the important part in the middle.' },
      { type: 'p', text: 'There is no universal right number, but the instinct to "send everything just in case" is usually wrong. A handful of high-quality passages typically beats a large pile, because precision in what you retrieve translates directly into precision in what the model answers. Tune this by watching real questions, the same way you tune chunk size.' },
      { type: 'h2', text: 'Hybrid beats dogma' },
      { type: 'p', text: 'There is no single best retriever, only trade-offs that run in opposite directions. Keyword search owns exact terms and misses paraphrase, embedding search owns meaning and fumbles exact strings, and most strong systems blend the two rather than betting on one, then re-rank and send only the best few passages. If your RAG system is mysteriously missing answers that obviously exist in your documents, the cause is very often a retrieval method that is blind to the kind of query you just sent, and hybrid search is usually the cure.' },
    ],
  },
  {
    id: 'kv-cache',
    ...
    body: [
      ...
      { type: 'h2', text: 'Remembering instead of recomputing' },
      ...
      { type: 'p', text: 'Generation splits into two phases, and the chatbot pause is the seam between them. First comes **prefill**: the model processes your entire prompt in one big pass and stores the intermediate results for every token into the cache. This is the heavy, all-at-once work, and it is the pause you see before the first word appears.' },
      { type: 'p', text: 'Then comes **decode**: the model generates new tokens one at a time, and for each one it reuses everything already in the cache and only computes the small new piece for the latest token. Decode is cheap per token, which is why, once it starts, the words stream out fast. Pause, then stream. Prefill, then decode. The cache is what turns the second phase from "redo everything" into "add one small thing."' },
      { type: 'h2', text: 'The cache, piece by piece' },
      { type: 'terms', items: [
      ...
      { type: 'h2', text: 'Why the cache changes your bill' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Recall how generation actually works. At each step the model does not choose the next word, it produces a score for every word it knows and turns those into probabilities.' },
      { type: 'p', text: 'Then a sampler draws one word from that distribution, favoring the likely ones without always taking the single most likely. Because there is a draw at every single step, two runs of the same prompt can diverge at the very first word and wander to completely different but equally valid answers. The model was never going to give you "the" answer, because it does not have one. It has a landscape of plausible answers and a habit of sampling from it.' },
      { type: 'h2', text: 'Walk a concrete case' },
      { type: 'p', text: 'Ask a model "suggest a name for a new coffee shop" and you might get "Daily Grind." Ask again and you might get "Bean & Gone." Neither is more correct than the other, and nothing changed between the two tries except which words the sampler happened to draw. Now notice that this is exactly what you want for that task. A naming tool that returned the identical suggestion every single time would be nearly useless, because the whole point is to explore options. The variability the tester flagged as a bug is, for a creative task, the feature people are paying for.' },
      { type: 'h2', text: 'A vocabulary for randomness' },
      { type: 'terms', items: [
        { term: 'Deterministic', def: 'always produces the same output for the same input, like a normal function or a calculator.' },
        { term: 'Probabilistic', def: 'produces outputs drawn from a distribution, so the same input can yield different results.' },
        { term: 'Greedy decoding', def: 'always taking the single highest-probability next word, which makes generation effectively deterministic.' },
        { term: 'Temperature', def: 'the knob that controls how much randomness the sampler uses. At zero it stops sampling and just takes the top word.' },
      ]},
...
` },
  ...
  },
  {
    ...
    body: [
      { type: 'p', text: 'Picture a feature where a model writes small functions on request. The team hits the question everyone hits: how do you know the code is any good. Two engineers eyeball each generated function and disagree. One says it looks clean, the other spots a case it probably misses.' },
      { type: 'p', text: 'They are both guessing, and they are about to ship guesses to users. Then someone points out the obvious thing they were all walking past: this is code. You do not have to argue about whether it works. You can run it.' },
      ...
      { type: 'h2', text: 'Walk a concrete example' },
      { type: 'p', text: 'Say you asked for a function that checks whether a number is prime. You do not squint at the logic. You write down cases you already know the answer to: 2 is prime, 7 is prime, 9 is not, 1 is not, and a large known prime is. You run the generated function on each one and compare.' },
      { type: 'p', text: 'If it returns the right verdict on all of them, your confidence jumps. If it calls 1 a prime, you have found a real bug in seconds, and notice that the code may have looked perfectly reasonable to a human skimming it. Running it caught what reading it missed.' },
      { type: 'h2', text: 'Naming the testing ideas' },
      { type: 'terms', items: [
        { term: 'Functional correctness', def: 'whether code actually produces the right output for given inputs, judged by running it rather than reading it.' },
        { term: 'Test case', def: 'one input paired with the output you expect, used to check the code.' },
        { term: 'Test suite', def: 'the full set of test cases. The more thorough it is, the more the result means.' },
        { term: 'Pass rate', def: 'the fraction of generated solutions that pass the whole suite. The headline number for this kind of evaluation.' },
        { term: 'Edge case', def: 'an unusual or boundary input where naive code tends to break, like zero, an empty list, or a negative number.' },
      ]},
      { type: 'h2', text: 'How this scales into a real evaluation' },
      { type: 'p', text: 'The same idea, scaled up, is how serious code benchmarks work. You collect a batch of programming problems, and for each one you write a hidden test suite that a correct solution must pass. You ask the model to solve all of them, run every solution against its tests in an automated harness, and report the **pass rate**: the share of problems it got fully right. Now you have a single honest number you can compare across models and across versions of your own prompt, and it came from execution, not from anybody\'s impression of the code. When a new model claims to be better at coding, this is usually the kind of measurement behind the claim.' },
      { type: 'h2', text: 'The two catches that bite people' },
      { type: 'p', text: 'The first catch is that your evaluation is only as good as your tests. Passing the suite does not prove the code is correct, it only proves the code is not wrong in any way your tests check. If you never test the empty input, a function that crashes on empty input still passes, and you ship the crash. This is why **edge cases** matter so much: the boundary inputs are exactly where weak code fails and lazy test suites stay silent. A green checkmark means "nothing I looked for is broken," not "everything works," and treating the first as the second is how confident teams ship bugs.' },
      { type: 'p', text: 'The second catch is safety. You are about to execute code that a model generated and that no human has fully vetted, which means you cannot run it directly on a machine you care about. Generated code can do destructive things by mistake, so it has to run inside a **sandbox**: an isolated environment with no access to your real files, network, or secrets, where the worst case is that the sandbox dies and you move on. Skipping this because "it is just a prime checker" is how an evaluation harness turns into a security incident. Isolate first, run second.' },
      { type: 'h2', text: 'Where else this works' },
      { type: 'p', text: 'The method reaches beyond functions that return numbers. Anything with a checkable answer can be graded this way. Did the generated SQL return the rows it was supposed to.' },
      { type: 'p', text: 'Does the regular expression match the strings it should and reject the ones it should not. Does the generated configuration actually parse. Whenever you can state success as "produces this specific result," you can stop debating quality and start measuring it, and your evaluation becomes something you trust rather than something you argue about.' },
      { type: 'p', text: 'So the team stopped eyeballing functions. They wrote test cases, including the nasty edge ones, ran every generated solution in a sandbox, and tracked the pass rate as they tuned their prompt. The disagreement between the two engineers evaporated, because there was now a number to point at instead of two opinions to defend. That is the quiet gift of any checkable task: evaluation gets to be a measurement, and you should take that gift every single time the task hands it to you.' },
    ],
  },
  {
    id: 'ai-as-a-judge',
    ...
    body: [
      ...
      { type: 'p', text: 'The quality of everything downstream rides on the rubric. A vague instruction like "rate this from 1 to 10" gives you mush, because the model has to invent its own definition of what the numbers mean. A specific rubric that spells out what earns each score gives you something repeatable.' },
      { type: 'h2', text: 'Walk a concrete setup' },
      { type: 'p', text: 'Suppose you are grading support replies. Instead of "score this reply," you give the judge a checklist: does the reply answer the actual question asked, is every factual claim supported by the help article provided, is it polite, is it under the length limit. You ask for a yes or no on each item and a short reason.' },
      { type: 'p', text: 'Now two different runs of the judge tend to agree, because they are answering concrete questions rather than guessing what a 7 means. You can even spot-check the judge by having a human grade the same fifty replies and seeing how often they match. That check is not optional, and it leads straight to the part people skip.' },
      { type: 'h2', text: 'The judge\'s vocabulary' },
      { type: 'terms', items: [
        { term: 'AI as a judge', def: 'using a strong model to score or compare other models\' outputs, in place of human graders.' },
        { term: 'Rubric', def: 'the explicit instructions telling the judge what good looks like. Specific rubrics give repeatable scores, vague ones give noise.' },
        { term: 'Position bias', def: 'a judge\'s tendency to favor whichever answer it sees first when comparing two, regardless of quality.' },
        { term: 'Verbosity bias', def: 'a judge\'s tendency to rate longer answers higher even when the extra length adds nothing.' },
        { term: 'Self-preference', def: 'a judge\'s tendency to prefer outputs written in a style similar to its own, including ones from the same model family.' },
      ]},
      { type: 'h2', text: 'The biases you have to design around' },
      { type: 'p', text: 'A model judge is not an objective instrument, it is another model, and it brings predictable thumb-on-the-scale tendencies. The best documented is **position bias**: when you ask a judge to pick the better of two answers, it tends to favor the one shown first, sometimes strongly, even if you swap which answer is actually better. There is also **verbosity bias**, where a longer answer gets a higher score just for being longer, so a padded reply beats a tight one that said the same thing.' },
      { type: 'p', text: 'And there is **self-preference**, where a judge rates outputs that look like its own writing more highly. None of these are exotic. They show up in ordinary use, and if you do not know about them they quietly corrupt your numbers.' },
  ...
  },
  {
    ...
    body: [
      { type: 'p', text: 'Picture a chatbot launched with clear safety training. Ask it for something harmful and it refuses politely, exactly as designed. Then within days, screenshots are circulating of the same bot happily giving the answer it just refused, because someone wrapped the request in a little story.' },
      { type: 'p', text: 'They did not hack a server or steal a password. They typed a clever paragraph into the same box every other user types into, and the model walked right past its own rules. That is **jailbreaking**, and understanding it is part of building anything that puts a model in front of the public.' },
      { type: 'p', text: 'The thing to get straight first is why this is even possible. A model\'s safety training is not a hard gate like a password check that either passes or fails. It is a strong tendency, learned from examples, to refuse certain kinds of requests. Tendencies can be pushed against.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'The first thing to clear up is that "build" and "buy" are not two options, they are a ladder with several rungs, and people collapse them into a false choice. At one end you call a hosted model through an API and write nothing but prompts.' },
      { type: 'p', text: 'A step up, you take an existing open model and run it yourself. Further up, you finetune an open model on your own examples. At the far end, you train a model from scratch on your own data and hardware. The cost, the expertise required, and the time all climb steeply as you move up the ladder, and so does the chance you end up worse off than where you started.' },
      ...
      { type: 'p', text: 'The day training ends, the frontier keeps moving and your owned model does not, a problem worth naming: capability debt. Owning a model really means paying to rebuild it on a schedule. So treat build-or-buy as a standing decision, not a one-shot one. Decide cheap now, and write down the triggers that would reopen the question: a price change, a new data rule, a volume threshold, an open model crossing your quality bar.' },
      { type: 'h2', text: 'How to decide without guessing' },
      { type: 'p', text: 'The way out of the argument is the same as everywhere else in this series: measure before you commit. Start at the cheapest rung that could plausibly work, usually a hosted model with a good prompt, and test it on your real tasks. If it meets the bar, you are done, and you just saved a project.' },
      { type: 'p', text: 'If it falls short, add retrieval and test again. Still short, try finetuning and test again. Only climb to the next rung when the one below it has actually failed a real test, not when you imagine it might. Each rung up costs more in money and expertise, so make the rung below prove it is insufficient before you pay for the next one.' },
      { type: 'p', text: 'The mirror-image failure deserves its own warning: buying blindly and wrapping nothing around it, no retrieval, no evaluation, no workflow, wastes the rung you are standing on. The teams that win with bought models are the ones that spend the saved budget on the integration.' },
      { type: 'p', text: 'The founder\'s team eventually did this in the right order and landed two rungs down from where they started: a hosted model with their documents wired in through retrieval, no training at all, shipped in days instead of months. They still "owned" the part that mattered, which was the product and the data, and they spent their engineering budget on the experience instead of on a worse copy of something they could rent. That is the quiet truth behind build versus buy. The serious-sounding choice is usually the wrong one, and the discipline is to make the cheap option prove it cannot work before you reach for the expensive one.' },
      { type: 'sources', items: [
        { title: 'Lu et al., "Buy versus Build an LLM: A Decision Framework for Governments" (2026), arXiv:2602.13033', url: 'https://arxiv.org/abs/2602.13033' },
        { title: 'DataHub Analytics, "Custom LLMs for Internal Enterprise Data: Build or Buy?"', url: 'https://datahubanalytics.com/custom-llms-for-internal-enterprise-data-build-or-buy/' },
      ]},
    ],
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'The problem is not the prompt. The problem is that they have no ruler. They are cutting without measuring, and the fix is to build the ruler first: an **evaluation pipeline**.' },
      ...
      { type: 'diagram', title: 'The evaluation pipeline, end to end',
        nodes: [
          { id: 'dataset', label: 'Eval dataset', icon: 'datastore', at: [0, 1] },
          { id: 'runner', label: 'Runner', icon: 'service', at: [1, 1] },
          { id: 'checks', label: 'Code checks', icon: 'service', at: [2, 0] },
          { id: 'judge', label: 'LLM judge', icon: 'model', at: [2, 1] },
          { id: 'human', label: 'Human review', icon: 'user', at: [2, 2] },
          { id: 'report', label: 'Score report', icon: 'datastore', at: [3, 1] },
          { id: 'gate', label: 'Ship gate', icon: 'service', at: [4, 1] },
        ],
        edges: [
          { from: 'dataset', to: 'runner', label: 'cases' },
          { from: 'runner', to: 'checks', route: 'vh' },
          { from: 'runner', to: 'judge', label: 'outputs' },
          { from: 'runner', to: 'human', route: 'vh' },
          { from: 'checks', to: 'report', route: 'hv' },
          { from: 'judge', to: 'report' },
          { from: 'human', to: 'report', route: 'hv' },
          { from: 'report', to: 'gate', label: 'scores' },
        ],
        groups: [{ label: 'Scorers', from: [2, 0], to: [2, 2] }],
...
` },
      ...
      { type: 'p', text: 'One more cut matters when your system has stages. A RAG feature, retrieval feeding generation, deserves a score for each stage as well as for the whole, because an end-to-end number can tell you something broke and still leave you guessing which half to fix. Score retrieval on whether the right passages came back, score generation on what the model did with them, and the end-to-end score stops being a mystery when it moves.' },
      { type: 'h2', text: 'Naming the moving parts' },
      { type: 'terms', items: [
        { term: 'Evaluation dataset', def: 'the fixed set of inputs you score every version against. Its representativeness decides whether the score means anything.' },
        { term: 'Golden set', def: 'inputs paired with known-good answers, the gold standard you grade against.' },
        { term: 'Scorer', def: 'the method that turns an output into a number: running it, comparing to a reference, or a model judge.' },
        { term: 'Code check', def: 'a deterministic test on an output, like schema validity or length. Free, instant, and always run first.' },
        { term: 'Stability rate', def: 'the share of repeated runs of the same case that pass. It exposes coin-flip behavior a single run hides.' },
        { term: 'Threshold', def: 'the score below which a change is blocked from shipping. Decided before you measure, not after.' },
        { term: 'Regression', def: 'a change that makes a previously-good case worse. The main thing the pipeline exists to catch.' },
        { term: 'Offline vs online evaluation', def: 'offline runs against your fixed dataset before shipping. Online measures real user outcomes after shipping. You need both.' },
      ]},
      { type: 'h2', text: 'The traps that make a pipeline lie to you' },
      { type: 'p', text: 'A pipeline can give you a confident number that is wrong, and there are two classic ways. The first is an unrepresentative dataset. If your two hundred cases are all easy and tidy while real traffic is messy, a high score tells you nothing about real performance, and you will ship a model that aces the test and fails the users. The dataset has to mirror reality, edge cases and ugliness included, and it has to be refreshed as real traffic shifts, or it slowly drifts away from the thing it is supposed to measure. A stale evaluation set is worse than none, because it gives false confidence.' },
      { type: 'p', text: 'The second trap is overfitting to your own test. If you keep tweaking the prompt until the score on your fixed dataset goes up, you eventually learn the quirks of that specific dataset rather than genuinely improving, the same way a student who memorizes last year\'s exam does not actually learn the subject. The defense is to keep some evaluation data held back and not look at it while you iterate, so it can give you an honest read at the end. The score is a proxy for real quality, and the moment you start optimizing the proxy for its own sake, it stops tracking the thing you care about.' },
      { type: 'p', text: 'There is a third trap the first two overshadow: trusting a single run. Ask a model the same question twice and you can get two different answers, so a case that "passed" once may fail the next three tries. A pipeline that runs everything once is measuring noise and calling it signal. Run each case several times, score the stability, and keep infrastructure errors in their own column instead of counting them as failures, or a flaky Tuesday at your API provider will read as a quality regression in your prompt.' },
      { type: 'h2', text: 'Gate it, then version everything' },
      { type: 'p', text: 'A pipeline that runs when somebody remembers is a dashboard, not a ruler. The step that changes behavior is wiring it into the same machinery that guards your code: every change to a prompt, a model choice, or a retrieval setting triggers the evaluation suite, the results land next to the change for review, and a score below the **threshold** blocks the ship, exactly the way a failing test blocks a merge. The threshold is agreed on before anyone measures, because a bar you set after seeing the score is not a bar, it is a rationalization.' },
      { type: 'p', text: 'Gating only works if you can trust what a score refers to, and that is a versioning problem. A score is meaningless on its own. It describes a specific prompt, a specific model at a specific temperature, a specific dataset, and a specific judge reading a specific rubric. Change any of those silently and the numbers stop being comparable across time, which is how a team ends up arguing about whether quality "really" dropped.' },
      { type: 'p', text: 'So version all of them together, record every one of them with every run, and store the results somewhere history survives. When the score moves, you want to spend your minutes fixing the cause, not reconstructing what changed.' },
      { type: 'h2', text: 'Offline is not the whole story' },
      { type: 'p', text: 'Everything so far is **offline evaluation**: running against a fixed dataset in a lab before you ship. It is essential and it is not sufficient, because no fixed dataset perfectly predicts how real people will use the thing. That is why mature teams pair it with **online evaluation**: once a change is live, they watch real outcomes, like whether users accepted the answer, asked a follow-up out of frustration, or escalated to a human. Offline evaluation tells you a change is safe enough to try. Online evaluation tells you whether it actually helped the people you built it for, and the two together close the loop that a single number never could.' },
      { type: 'callout', title: 'The loop that compounds', text: 'Every production failure becomes a permanent test case. File the incident, add the case to the dataset, and that exact mistake can never ship quietly again. A team that does this for a year owns an evaluation set no vendor could sell them.' },
      ...
      { type: 'sources', items: [
        { title: 'Braintrust, "The complete guide to LLM evaluation"', url: 'https://www.braintrust.dev/articles/llm-evaluation-guide' },
        { title: 'Chrome for Developers, "Build an evaluation pipeline"', url: 'https://developer.chrome.com/docs/ai/evals/pipeline' },
        { title: 'Datadog, "LLM evaluation frameworks"', url: 'https://www.datadoghq.com/knowledge-center/llm-evaluation-frameworks/' },
        { title: 'AWS Machine Learning Blog, "Build an automated generative AI evaluation pipeline with Amazon Nova"', url: 'https://aws.amazon.com/blogs/machine-learning/build-an-automated-generative-ai-solution-evaluation-pipeline-with-amazon-nova/' },
        { title: 'NVIDIA Technical Blog, "Mastering LLM techniques: evaluation"', url: 'https://developer.nvidia.com/blog/mastering-llm-techniques-evaluation/' },
      ]},
    ],
  },
  {
    ...
    body: [
      { type: 'p', text: 'Picture a team choosing a model the obvious way: they look up a leaderboard, pick the model sitting at the top, and wire it in. It flops. The answers are slow, they ramble past the length their UI can show, and they ignore half the formatting instructions the team carefully wrote.' },
      { type: 'p', text: 'The model that "won" lost at the only contest that mattered, which was their product. The mistake was treating a single overall ranking as if it measured fitness for their job. It did not. It measured a general average, and your job is never the general average.' },
      ...
      { type: 'p', text: 'They also found that before this standardization, prominent models had been compared on under a fifth of the same test scenarios, so even the "overall" rankings people quoted were built from tests that barely overlapped. A single capability score is not summarizing a coherent thing. It is averaging a committee that disagrees.' },
      ...
      { type: 'p', text: 'The subject is ordinary, so deep domain knowledge is not the bottleneck. Written out like that, the team\'s priorities are nothing like the leaderboard\'s priorities, and a model that ranks lower overall but nails instructions and responds fast is plainly the better choice for them. The leaderboard was answering a question they never asked.' },
      { type: 'diagram', title: 'Which capability gates your task?', root: {
        label: 'Is a person waiting on the answer live?', color: 'purple', children: [
          { edge: 'yes', node: { label: 'Latency and cost lead', color: 'yellow' } },
          { edge: 'no', node: {
            label: 'Does the output feed code or a fixed UI?', color: 'blue', children: [
              { edge: 'yes', node: { label: 'Instruction-following leads', color: 'yellow' } },
              { edge: 'no', node: {
                label: 'Is the subject specialized?', color: 'blue', children: [
                  { edge: 'yes', node: { label: 'Domain knowledge leads', color: 'yellow' } },
                  { edge: 'no', node: { label: 'Generation quality leads', color: 'yellow' } },
                ] } },
            ] } },
        ],
      ...
      { type: 'h2', text: 'Naming what you are measuring' },
      { type: 'terms', items: [
        { term: 'Domain knowledge', def: 'how much the model knows about your specific subject area. Matters most for specialized fields.' },
        { term: 'Generation quality', def: 'whether the output is accurate, coherent, and genuinely useful for the task.' },
        { term: 'Instruction-following', def: 'whether the model obeys what you actually told it, including constraints like length, format, and tone.' },
        { term: 'Cost and latency', def: 'the price per call and the time to respond. Often the deciding factor for live, high-volume features.' },
        { term: 'Benchmark', def: 'a standardized test producing a score. Useful as a rough signal, dangerous as a final verdict for your specific job.' },
        { term: 'Preference leaderboard', def: 'a ranking built from people voting between anonymous model answers. It measures average helpfulness for its voters, not fitness for your task.' },
      ...
      { type: 'p', text: 'The voters skew toward enthusiasts and researchers rather than everyday users, the prompt mix is whatever the crowd brings rather than anyone\'s production traffic, and the votes measure helpfulness, not safety or reliability. None of that is a scandal. It is a well-run answer to a question that is simply not "which model should power your product."' },
      ...
      { type: 'p', text: 'And popular chat models have been shown to regenerate well-known evaluation datasets nearly verbatim on request, which is hard to do without having trained on them. When a model has seen the test, its score reflects familiarity as much as ability, and since the biggest models keep their training data secret, you usually cannot check. Benchmarks remain a cheap first filter. They must never be the last word, because you cannot audit what they secretly rehearsed.' },
...
` },
      ...
      { type: 'diagram', nodes: [
        ...
        { label: 'Shortlist', detail: 'two or three candidates' },
      ...
      { type: 'sources', items: [
        ...
        { title: 'Zhou et al., "Instruction-Following Evaluation for Large Language Models" (IFEval, 2023)', url: 'https://arxiv.org/abs/2311.07911' },
        { title: 'Chiang et al., "Chatbot Arena: An Open Platform for Evaluating LLMs by Human Preference" (2024)', url: 'https://arxiv.org/abs/2403.04132' },
        { title: 'Sainz et al., "NLP Evaluation in Trouble: On the Need to Measure LLM Data Contamination" (EMNLP 2023 Findings)', url: 'https://arxiv.org/abs/2310.18018' },
      ]},
    ],
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: 'The text is already broken before the model reads it' },
      { type: 'p', text: 'Here is the part most guides skip. A scanned or photographed document is not text, it is an image, and something has to turn those pixels into characters first.' },
      ...
      { type: 'image', src: '/blog-images/extraction/invoice-annotated.jpg', alt: 'A sample invoice with the header marked read correctly and the price table marked scrambled by OCR', caption: 'The real invoice we tested. Green is what the model read correctly. Red is the price table, where OCR scrambled the columns and the model guessed.' },
      { type: 'p', text: 'To see why the model struggled, look at what it actually received. This is the OCR text of the item table, and the prices have floated away from the items they describe:' },
      { type: 'code', lang: 'text', title: 'OCR text the model received (excerpt)', code: `ITEMS
No. Description Qty
...
34,99` },
      { type: 'p', text: 'A person looking at the printed invoice has no trouble, because the eye follows each row across and pairs the dress with its price. The model never saw the printed invoice. It saw the block of text above, where a row\'s price can sit a dozen lines away from the row\'s description, separated from the other prices only by blank lines. Pairing them back up is guesswork, and the model guesses the way it always does under uncertainty, by producing something that looks plausible. That is the whole reason a clean-looking invoice can still produce a broken answer.' },
      { type: 'h2', text: 'Two prompts, two different failures' },
      { type: 'p', text: 'We tried two prompts on that scrambled text. The first is the loose one everybody starts with: "extract the important information from this invoice." It hands every decision to the model, and the model wrote back a friendly report. Here is the surprising part: in that chatty paragraph, it got all seven prices right and the totals right, pairing each price to the correct item, even though the text was a mess. It reasoned its way through. The catch is that the answer is prose, and no program can reliably pull structured data out of a paragraph:' },
      { type: 'image', src: '/blog-images/extraction/weak-output.jpg', alt: 'The loose prompt output, a chatty English summary that lists the correct prices', caption: 'The loose prompt got all seven prices and the totals right, but as prose a program cannot parse.' },
      { type: 'p', text: 'So we tightened it into a specification: name every field, return one JSON object and nothing else, write null instead of guessing, use only what the document shows. This is the advice you read everywhere, and it gave us exactly the clean, parseable shape we asked for. It also got the prices wrong. The same model that had just reasoned out every price in prose now returned 1234.56 for the first item and guessed the rest:' },
      { type: 'image', src: '/blog-images/extraction/strong-output.jpg', alt: 'The strict prompt output, clean JSON with the wrong prices', caption: 'The strict prompt: clean, parseable JSON, and the prices are wrong. Structure is not accuracy.' },
      { type: 'h2', text: 'Structure is not accuracy' },
      { type: 'p', text: 'That flip is the real lesson, and it runs against the usual advice. Forcing the model into a rigid shape did not make it more accurate, it made it less. Two things went wrong the moment we demanded strict JSON. First, we took away the room to reason.' },
      { type: 'p', text: 'The loose prompt let the model think out loud and work the scrambled table into the right pairs; the strict prompt made it emit JSON immediately, with no space to reason, so on the hard rows it simply guessed. Second, a number leaked. We had written a formatting rule, "convert a decimal comma to a dot, so 1.234,56 becomes 1234.56", and the model lifted 1234.56 straight into the first item as if it were a real price. Under the strict format the whole price table came apart:' },
      { type: 'ul', items: [
      ...
      { type: 'p', text: 'So structured output buys you one thing and not the other. It guarantees a shape your code can read. It does not guarantee the values are right, and it can make them worse by taking away the reasoning the model needs on a hard input. A schema is a promise about the container, not about the contents.' },
      { type: 'h2', text: 'Where the accuracy actually comes from' },
      { type: 'p', text: 'If the strict prompt broke on the scrambled table, then the thing to fix is the table, not the wording of the prompt. The scramble was an OCR artifact: the default settings read the columns apart. One flag fixed it, telling the OCR to keep each row on a single line:' },
      { type: 'code', lang: 'bash', title: 'layout-preserving OCR', code: `tesseract invoice.png out --psm 6 -c preserve_interword_spaces=1` },
      { type: 'p', text: 'With the rows kept intact, we ran the exact same strict prompt again, unchanged, and this time the model returned every price correctly and the totals to the cent. Same model, same prompt, one change to the input:' },
      { type: 'code', lang: 'text', title: 'the strict prompt, before and after fixing the OCR', code: `default OCR  ->  unit prices: 1234.56, 1249.00, 1250.00, 2500.00, ...     (wrong)
...
` },
      ...
      { type: 'terms', items: [
        { term: 'Information extraction', def: 'pulling specific structured fields out of unstructured text, like getting a total and a date out of an invoice.', url: 'https://en.wikipedia.org/wiki/Information_extraction' },
        { term: 'OCR', def: 'optical character recognition: turning an image of a document into text. Its mistakes happen before the model reads anything.', url: 'https://en.wikipedia.org/wiki/Optical_character_recognition' },
        { term: 'Schema', def: 'the exact shape the output must take: which fields exist and what type each one is.', url: 'https://json-schema.org/' },
        { term: 'Grounding', def: 'restricting the model to facts actually present in the source text, so it does not invent fields from general knowledge.', url: 'https://en.wikipedia.org/wiki/Hallucination_(artificial_intelligence)' },
        { term: 'Validation', def: 'checking the output against the schema and simple rules after the fact, and rejecting anything that does not conform.', url: 'https://json-schema.org/learn/getting-started-step-by-step' },
      ]},
      { type: 'p', text: 'So the honest lesson is humbler than "write a stricter prompt." A schema gives you a shape you can parse, which is worth having, but it does not give you correct values and it can quietly cost you accuracy on a hard input. The values come from what the model can actually read, which means the real fight is usually upstream at OCR, and from checking the result afterward. Name your fields and demand a shape, but fix the input first, let the model reason where it needs to, and validate every number before you trust it. That is what lets a document reader survive contact with the real, messy, badly-scanned world, where the invoice you never tested is always the next one through the door.' },
      { type: 'sources', items: [
        { title: 'Invoice image: Voxel51 "high-quality-invoice-images-for-ocr" dataset (Hugging Face)', url: 'https://huggingface.co/datasets/Voxel51/high-quality-invoice-images-for-ocr' },
        { title: 'Optical character recognition (Wikipedia)', url: 'https://en.wikipedia.org/wiki/Optical_character_recognition' },
        { title: 'JSON Schema', url: 'https://json-schema.org/' },
        { title: 'Multilingual OCR-Aware Fine-Tuning and Prompt-Guided CoT Reasoning (arXiv:2605.16409)', url: 'https://arxiv.org/abs/2605.16409' },
      ]},
    ],
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'If tomorrow you care about something you never considered today, you just describe the new thing. No retraining, no labeling, no fresh dataset. The vocabulary is as wide as language itself.' },
      { type: 'p', text: 'This works because the model learned meaning rather than a list. It was trained on enormous numbers of image-and-text pairs, so it built up a shared sense of how words relate to what is in a picture. The phrase "lifting machine" lands near the idea of a forklift in that learned sense, even if the exact word forklift never came up for this particular image, which is why a description the model has never seen can still find the right object.' },
      { type: 'h2', text: 'Working one case end to end' },
      { type: 'p', text: 'Go back to the warehouse, this time with an open-vocabulary model like NVIDIA\'s LocateAnything-3B. You hand it the photo and the single word "forklift." It returns a box, given by corner coordinates, around the machine it earlier had no name for. You change the phrase to "the fire extinguisher on the wall," and without touching the model, without adding a fire-extinguisher category, you get a box around the red cylinder by the door. You ask for "every pallet on the floor" and it boxes each one.' },
      { type: 'p', text: 'The picture never changed and the model\'s weights never changed. The only thing that changed was the sentence you gave it. That is the entire shift: the query moved out of a fixed list baked in at training time and into a line of text you write at the moment you ask.' },
      { type: 'h2', text: 'The detector\'s glossary' },
      { type: 'terms', items: [
        { term: 'Object detection', def: 'finding things in an image and drawing a box around each one, with a label for what it is.' },
        { term: 'Bounding box', def: 'the rectangle that marks where an object sits, given by its corner coordinates.' },
        { term: 'Closed vocabulary', def: 'a fixed list of categories chosen before training. The model can only ever find things on this list.' },
        { term: 'Open vocabulary', def: 'finding an object from a free-text description, including kinds of thing the model was never explicitly trained to name.' },
        { term: 'Visual grounding', def: 'tying a piece of language to the exact spot in an image it refers to. You give a phrase, the model points.' },
        { term: 'Vision-language model (VLM)', def: 'a single model that takes in both an image and text and reasons over the two together.' },
      ]},
      { type: 'h2', text: 'How LocateAnything actually does it' },
      { type: 'p', text: 'LocateAnything-3B is a vision-language model with about three billion parameters. Two parts sit inside it: a vision encoder that turns the image into a form the model can reason over, and a language model that handles the words.' },
      { type: 'p', text: 'The clever part is the output. Instead of a separate detection head that emits boxes, the model simply writes text, and the box is part of that text. Ask it to locate the forklift and it produces the label together with coordinate tokens, a short tagged span carrying four numbers for the corners. Those numbers are normalized to a fixed range from 0 to 1000 regardless of the image\'s real pixel size, so the same output format works whether the photo is tiny or huge, and your code scales them back to pixels. A point, used when you want a location rather than a region, is written the same way with a single coordinate pair.' },
      { type: 'p', text: 'What gives it such a wide vocabulary is the sheer range of what it was trained on. NVIDIA reports building it from roughly twelve million images carrying over a hundred and thirty million text queries and hundreds of millions of boxes, drawn from natural scenes, robotics, driving, graphical user interfaces, and documents. Because the training spanned so many kinds of image and so many ways of describing things, one model can locate ordinary objects, dense clusters of small ones, on-screen buttons, and lines of text in a scanned document, all through the same describe-and-point interface.' },
      { type: 'h2', text: 'Parallel box decoding, and why speed matters' },
      { type: 'p', text: 'This design has a catch it has to fight. A language model normally writes one token at a time, each token depending on the ones before it, which is called autoregressive decoding. That is fine for a sentence, but a box is four numbers, and a crowded scene might hold dozens of boxes, so spelling out every coordinate digit one after another gets slow. LocateAnything\'s headline idea for speed is parallel box decoding: rather than emit a box digit by digit, it predicts the full set of coordinates for a box in a single parallel step. NVIDIA reports this runs up to about two and a half times faster than the token-by-token approach.' },
      { type: 'p', text: 'It exposes this as a speed-versus-care dial with three settings. A fast mode always takes the parallel shortcut. A slow mode falls back to plain one-at-a-time decoding, which is more careful on hard cases. The default is a hybrid that takes the fast path first and drops back to the careful path only for boxes it is unsure about, then returns to the fast path. In the warehouse, a clean shot of a single forklift sails through on the fast path, while a cluttered shelf of overlapping parts is where the hybrid earns its keep by slowing down just for the tricky boxes.' },
      { type: 'h2', text: 'Try the idea in code' },
...
` },
      ...
      { type: 'p', text: 'If you only ever need to find people and cars, and you need it in a few milliseconds on a cheap chip, a classic detector trained on those classes will beat a three-billion-parameter model on speed and cost. Open vocabulary buys you flexibility, and you pay for it in compute.' },
      { type: 'p', text: 'Two practical limits are worth naming plainly. LocateAnything-3B ships under an NVIDIA license for non-commercial research use, so you can study it and build prototypes, but dropping it inside a product you sell is not permitted without different terms. And a model that points at things in still photos is not the same as a perception system safe enough to drive a car. The Uber case earlier shows the stakes of getting localization wrong in the real world, and nothing here should be read as suggesting a research VLM is ready for that job. It is a flexible tool for finding described things in images, which is already plenty.' },
      { type: 'h2', text: 'Detection is a system, not a model' },
      { type: 'p', text: 'A traditional detector is a closed list: fast, cheap, and blind to anything it was not trained to name. Open-vocabulary grounding swaps the list for language, so the question moves from "is this one of my eighty categories" to "here is a sentence, find what it describes," and you can change the sentence any time without retraining. LocateAnything-3B is one strong recent example, a vision-language model that writes boxes as text and leans on parallel decoding to stay fast.' },
      { type: 'p', text: 'Reach for this kind of model when the things you need to find are open-ended, rare, or unknown until a user asks. Reach for a plain detector when the categories are few, fixed, and raw speed is the priority. The skill is knowing which of those two worlds your problem actually lives in.' },
    ],
  },
];
