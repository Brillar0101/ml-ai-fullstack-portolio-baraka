...
export const POST = {
  ...
  body: [
    ...
    { type: 'terms', items: [
      ...
      { term: 'Memory', url: 'https://langchain-ai.github.io/langgraph/concepts/memory/', def: 'Short-term memory is what happens inside this session: the transcript plus the notes and tool output accumulated so far. Long-term memory is dedicated storage that carries facts across sessions. Your Redis preference lived there.' },
    ...
    ctx = {
        "instructions": load_system_prompt(),        # 1 role, objective, requirements
        "examples":     load_few_shot_examples(),    # 2 behaviour + response demos
        "tools":        list_tool_schemas(),         # 5 descriptions + parameters
        "guardrails":   load_guardrail_rules(),      # 6 input/action/output rules
        "long_term":    recall_facts(request.user),  # 4 memory across sessions
    }
    used = sum(count_tokens(v) for v in ctx.values())

    # Knowledge gets a capped slice of what is left.
    remaining = budget - used
    ctx["knowledge"] = search_repo(request.text, limit=remaining // 3)  # 3

    # Short-term memory takes the rest, newest first.
    ctx["short_term"] = recent_messages(request.session) + request.tool_results  # 4

    return trim_oldest(ctx, budget)  # drop stale tool output if over budget`, },
    { type: 'p', text: 'Notice that the six are named fields, not a single string. Keeping them separate in code is the point. It lets you cache the authored layers, cap the fetched ones, and evict from the growing ones without touching the others. An agent that concatenates everything into one prompt loses the ability to reason about any of this.' },
    { type: 'p', text: 'The ordering matters as much as the split. Models pay closest attention to text near the start and near the end of the window, and least to whatever sits in the deep middle. So the instructions and guardrails go up top where they anchor everything, the live conversation and the freshest tool results sit near the bottom where they stay salient, and bulky retrieved documents go in between where a little inattention costs the least. When you assemble context, you are not just choosing what to include. You are choosing where in the window each piece lands, and that placement quietly shapes how much the model actually uses it.' },
    ...
    { type: 'ul', items: [
      'Instructions fail by being vague, contradictory, or buried. If the requirements say "be concise" and also "explain your reasoning in full," the agent will pick one at random. If a rule sits at the very bottom under thousands of tokens of history, the model may weight it lightly. Fix: keep the role, objective, and requirements short, non-conflicting, and near the top.',
      'Examples fail by teaching the wrong lesson. A stale example that contradicts a newer instruction wins surprisingly often, because the model imitates what it is shown more readily than what it is told. Fix: keep examples current, few, and consistent with the rules they sit beside.',
      'Knowledge fails two ways. Retrieval fetches the wrong documents, so the agent answers from irrelevant text, or it fetches too many, drowning the real answer in noise. Both look like the model being dumb. Usually the retriever is the culprit, not the model.',
      'Memory fails at both ends. Long-term memory remembers the wrong thing, or the fact exists but nothing loads it when it matters. Save "the user prefers tabs" from one offhand comment and it haunts every future session. Short-term memory fails by overflowing: a long session exceeds the window, the oldest turns with the original goal get dropped, and the agent drifts. Fix: durable facts only in the store, and summarize old turns instead of hard-cutting them.',
      'Tools fail when the description is fuzzy or the parameters are loose. A tool called "search" with a free-form string argument invites the model to pass garbage. The agent then calls it wrong, gets an error back as a result, and often does not recover. Fix: name tools for what they do and constrain parameters tightly.',
      'Guardrails fail in both directions. Too loose, and an instruction hidden inside a fetched document walks straight into the model, or malformed output reaches production unchecked. Too tight, and the agent cannot do its job at all. Fix: clean and tag inputs, allowlist the actions the agent may take, and validate outputs against a schema before shipping them.',
    ]},
