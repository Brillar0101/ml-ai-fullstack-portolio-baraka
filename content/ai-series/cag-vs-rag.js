export const POST = {
  ...
  body: [
    { type: 'p', text: 'An employee asks the internal HR bot a simple question. "How many days of paternity leave do I get, and does it change if I have been here less than a year?" The bot answers with the paternity number, sounds confident, and stops. It never mentions the tenure rule, which lives one paragraph down in the same policy.' },
      { type: 'p', text: 'The employee takes the answer at face value, plans around it, and later finds out the real number was different for someone in their first year. The policy document had the full answer the whole time. The bot just never saw the part that mattered.' },
    {
      type: 'p',
    ...
    { type: 'p', text: 'Here is the intuition that fixes it. Imagine a new hire at a help desk. In one setup, every policy sits in a filing cabinet down the hall, and for each question the hire runs to the cabinet, grabs the two or three pages they think are relevant, and answers from those.' },
      { type: 'p', text: 'If they grab the wrong pages, they answer wrong, even though the right page exists. In the other setup, the entire policy binder is open flat on the desk in front of them. Every question gets answered against the full binder. No trip to the cabinet, no guessing which pages to pull, no chance of leaving the tenure rule in the drawer.' },
    { type: 'p', text: 'The second setup is **CAG**, Cache-Augmented Generation. Instead of retrieving a slice of your knowledge for each query, you load the entire knowledge base into the model\'s context one time, and every question is answered against all of it. There is no retrieval step to get wrong.' },
      { type: 'p', text: 'The tradeoff is obvious the moment you say it out loud. This only works if the whole binder actually fits on the desk. For a company handbook that is a few dozen pages, it fits. For a legal database of ten million documents, it does not, and you are back to the filing cabinet.' },
    {
      type: 'h2',
      text: 'Walking one question through both systems'
    },
    { type: 'p', text: 'Take the paternity leave question and run it through each. In the RAG version, the query is turned into a vector, compared against the stored policy chunks, and the top matches come back. Say the retriever returns the three highest-scoring chunks.' },
      { type: 'p', text: 'If the tenure clause ranks fourth, it is silently dropped. The model receives the question plus three chunks and writes a fluent, incomplete answer. Nothing errored. The context was just missing a piece.' },
    {
      type: 'p',
...
` },
    ...
    { type: 'p', text: 'Back to the HR bot. Swapping it from RAG to CAG did not make the model smarter. It removed the step that was throwing away the answer. Forty pages of policy fit inside a long context window with room to spare, and the handbook is updated a few times a year, not a few times an hour.' },
      { type: 'p', text: 'Both conditions held, so preloading was the right call. The takeaway is not that CAG beats RAG. It is that retrieval is a tool you can choose to skip, and for a small, stable body of knowledge, skipping it removes an entire class of failure for free.' },
    {
      type: 'sources',
      items: [
