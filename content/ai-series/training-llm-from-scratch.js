export const POST = {
  ...
  body: [
    ...
    { type: 'lab', packages: ['numpy'], height: 460,
        ...
        caption: 'Perplexity is just exp(loss), and it reads as how many words the model is effectively choosing between. Training is the act of moving the first number toward the second.',
        code: `import numpy as np

# The next-token objective, small enough to read. No framework: just the shift
# and the cross-entropy that every language model is trained to push down.
...
def next_token_loss(logits, tokens):
    ...
    probs = softmax(preds)
    B, T, V = probs.shape
    picked = probs.reshape(B * T, V)[np.arange(B * T), targets.reshape(-1)]
    return float(-np.log(picked + 1e-12).mean())    # cross-entropy

VOCAB = ["the", "cat", "sat", "on", "mat"]
tokens = np.array([[0, 1, 2, 3, 4]])       # "the cat sat on mat"

rng = np.random.default_rng(0)
random_logits = rng.normal(size=(1, 5, len(VOCAB)))

# A model that has learned this sentence puts its mass on the right next token.
trained_logits = np.full((1, 5, len(VOCAB)), -2.0)
for pos, nxt in enumerate(tokens[0][1:]):
    trained_logits[0, pos, nxt] = 6.0

for label, logits in [("untrained (random)", random_logits),
                      ("trained on this sentence", trained_logits)]:
    loss = next_token_loss(logits, tokens)
    print("%-26s loss %.4f   perplexity %7.2f" % (label, loss, np.exp(loss)))

print()
print("Perplexity is just exp(loss), and it reads as 'how many words is the")
print("model effectively choosing between at each step'. The untrained model")
print("is still choosing between several options. The trained one has nearly")
print("made up its mind. Training is the act of moving the first number")
print("toward the second, over and over, on a great deal of text.")

# Try it: lower 6.0 to 1.0 in trained_logits, so the model is only mildly
# confident, and watch perplexity climb back toward the vocabulary size.
` },
    {
      type: 'p',
