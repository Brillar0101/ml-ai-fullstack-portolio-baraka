export const POST = {
  ...
  body: [
    ...
    { type: 'p', text: 'The first instinct was to argue about the prompt. Someone added a line telling the agent to check for related files. Someone else rewrote the system message to be firmer about finishing the whole task.' },
      { type: 'p', text: 'Every change felt reasonable in the moment, and none of them moved the number. The team was steering blind, tweaking words and hoping the next batch of tickets would go better. The thing they were missing was sitting right there in their logs.' },
    {
      type: 'h2',
      text: 'An agent is not a build you ship once'
    },
    { type: 'p', text: 'Here is the shift that changed how they worked. A traditional program is mostly done when it passes its tests.' },
      { type: 'p', text: 'An agent is different. It meets a stream of real requests you never wrote down, in an order you never planned, and its behavior drifts as the model, the tools, and the users all change around it. You do not ship it and walk away. You put it in front of real traffic, watch what it actually does, and feed what you learn back into the next version. That feedback path is the whole game, and it works like a flywheel: slow to start, but each turn makes the next one easier.' },
    { type: 'p', text: 'The fuel for that flywheel is trace data. Every time the agent runs, it leaves behind a record of what it thought, which tools it called, what arguments it passed, and how the run ended. Most teams collect this and never look at it.' },
      { type: 'p', text: 'The insight is that your production logs are not just for debugging one broken request. They are a supply of real failure cases, written by real users, that no synthetic test set could invent. If you mine them well, they tell you exactly what to fix next.' },
    {
      type: 'h2',
      text: 'The loop that turns failures into a fix that sticks'
    },
    { type: 'p', text: 'The flywheel has a small number of stages, and the point is that you keep going around it rather than stopping after one lap. You capture traces from production. You look through them and sort the failures by what went wrong. You take the clearest failures and turn them into permanent test cases.' },
      { type: 'p', text: 'You change a prompt, a tool, or some logic to fix that class of problem. You re-run your test set to confirm the fix helped and broke nothing else. You deploy. Then you capture the next batch of traces, because the fix will surface new edge cases you could not see before.' },
    {
      type: 'diagram',
      nodes: [
    ...
    { type: 'p', text: 'The team stopped arguing about prompts and pulled a week of traces instead. They filtered to failed runs, which for them meant a pull request that got rejected or reverted. Reading through them, a pattern jumped out fast.' },
      { type: 'p', text: 'In almost every failure, the agent had opened a file, edited it, and stopped, without ever searching for the other files that referenced what it just changed. The bad plan was not random. It was the same missing step every time: the agent never looked for downstream callers.' },
    {
      type: 'p',
    ...
    { type: 'p', text: 'That ordering is easy to get backward, so it is worth dwelling on. The tempting move is to spot the pattern, feel confident, and go straight to the code change.' },
      { type: 'p', text: 'The problem is that you then have no way to measure whether you actually helped. You are back to steering by vibes, the same trap that ate the first week. Writing the eval cases first flips that.' },
      { type: 'p', text: 'You now have twelve runs that fail today, on purpose, and a fix is only real when they pass. It also protects you later. Six weeks from now, when someone rewrites that system prompt for an unrelated reason, these twelve cases stand guard and fail loudly if the old behavior sneaks back in.' },
    {
      type: 'p',
...
` },
    ...
    { type: 'p', text: 'With the twelve cases in place, add the search tool and the prompt rule, then re-run the whole eval set. Most of the twelve now pass.' },
      { type: 'p', text: 'The ones that still fail do so for a different reason, which is fine, because now those are named and tracked too. Crucially, you can also see that nothing which used to pass has broken. Ship it, and the next week of traces surfaces a new, smaller problem, and the loop goes around again. The point is not the score. It is that every step is now a measurement instead of a guess.' },
    {
      type: 'h2',
