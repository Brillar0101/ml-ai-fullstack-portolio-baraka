export const POST = {
  ...
  body: [
    { type: 'p', text: 'Picture a support agent that has behaved for weeks. It read a customer question, pulled a few relevant help articles, and wrote a reply. Cost was steady and boring, which is exactly what you want. Then one Tuesday the bill for a single day came in at three times the usual number.' },
      { type: 'p', text: 'Nothing had crashed. No alarm had gone off. The agent had answered every ticket correctly.' },
      { type: 'p', text: 'What changed was invisible from the outside: on certain tricky tickets the agent had started looping, retrying its own reasoning again and again, and on every single loop it re-sent the same large block of retrieved documents. The work looked the same. The token count did not.' },
    {
      type: 'p',
    ...
    { type: 'p', text: 'Think about riding in a taxi with the fare display covered by a cloth. The ride feels fine. You get where you are going. You have no idea what it costs until the driver pulls the cloth off at the end.' },
      { type: 'p', text: 'If the route was short you are relieved, and if the driver circled the block twenty times you are furious, but either way you found out too late to do anything about it. Most teams run their language model spend exactly this way. The request goes out, the answer comes back, and the fare is hidden until billing day.' },
    {
      type: 'p',
    ...
    { type: 'p', text: 'It also helps to notice which pieces are steady and which ones swing. The system prompt is close to fixed, so it adds a predictable floor to every call. Retrieved context and history are the movers.' },
      { type: 'p', text: 'They change from request to request and from session to session, which means most of the surprises in your bill come from them. When the support agent looped, it was the retrieved context that hurt, because that block was large and it went out again on every pass. If you know in advance which parts of the stack are volatile, you know where to look the moment a chart moves, instead of guessing across the whole request.' },
    {
      type: 'terms',
      items: [
    ...
    { type: 'lab', height: 460,
        ...
        caption: 'One feature is 99 percent of the bill, and it is the one pasting the most context into every call. The 30x input-to-output ratio underneath is where that money actually goes. An untagged invoice shows a single number and no way to act on it.',
...
` },
    { type: 'p', text: 'The prices here are placeholders, so read the current numbers off your provider page before you trust them. What matters is the shape.' },
      { type: 'p', text: 'Notice the tags for feature and customer. Those are what make the number attributable. Once every request carries them, you can group your spend by feature to see which one is expensive, or by customer to see whether one heavy account is quietly eating the margin on your flat-rate plan. Without tags you have one big total and no way to ask where it came from.' },
    {
      type: 'p',
    ...
    { type: 'p', text: 'Visibility pays off because it points you at the right fix instead of a guess. If a chart shows one feature dominating your input tokens, caching repeated context or trimming the retrieved passages down to what the answer needs can cut that line hard. If cheap, easy requests are being sent to your most expensive model, route the simple ones to a smaller model and keep the big model for the hard cases.' },
      { type: 'p', text: 'If output tokens are the problem, ask for shorter answers or cap the maximum length. None of these are clever. They are only obvious once the meter is on the wall and you can see which number to attack.' },
    {
      type: 'p',
